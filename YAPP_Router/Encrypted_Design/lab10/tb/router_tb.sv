...
class router_tb extends uvm_component;
    ...
    function void connect_phase(uvm_phase phase);
        ...
        env.agent.monitor.item_collected_port.connect(ref_env.yapp_export        );

        hbus.bus_monitor.item_collected_port.connect(ref_env.hbus_export        );

        ch0.monitor.item_collected_port.connect(ref_env.ch0_export        );

        ch1.monitor.item_collected_port.connect(    ref_env.ch1_export        );

        ch2.monitor.item_collected_port.connect(            ref_env.ch2_export        );  
