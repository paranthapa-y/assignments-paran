/*-----------------------------------------------------------------
File name     : router_disbl_vseq.sv
Description   : Virtual sequence library for high-level test stimulus generation
Notes         :
-------------------------------------------------------------------
-----------------------------------------------------------------*/

class router_disbl_vseq extends uvm_sequence;
    `uvm_object_utils(router_disbl_vseq)
    ...
    yapp_012_seq              seq012;
    six_yapp_seq              seq6;
    function new(string name ="router_disbl_vseq");
        super.new(name);
    endfunction

    task body();
    
        `uvm_info(get_type_name(), "Starting router_disbl_vseq", UVM_LOW)
